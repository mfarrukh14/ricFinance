  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 dark:text-slate-100">Fiscal Years</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Manage financial year periods</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {fiscalYears.map((fy) => (
            <div
              key={fy.id}
              className={`bg-white dark:bg-slate-900 rounded-2xl shadow-sm border-2 p-6 hover:shadow-lg transition-all ${
                fy.isCurrent ? 'border-teal-500 ring-4 ring-teal-500/10' : 'border-slate-200 dark:border-slate-800'
              }`}
            >
              <div className="flex items-start justify-between mb-4">
                ...
                {fy.isCurrent && (
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-teal-100 dark:bg-teal-950/40 text-teal-700 dark:text-teal-200 text-xs font-semibold rounded-full">
              ...
              <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-4">FY {fy.year}</h3>

              <div className="space-y-2 mb-6">
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500 dark:text-slate-400">Start Date</span>
                  <span className="text-slate-700 dark:text-slate-200 font-medium">{formatDate(fy.startDate)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500 dark:text-slate-400">End Date</span>
                  <span className="text-slate-700 dark:text-slate-200 font-medium">{formatDate(fy.endDate)}</span>
                </div>
              </div>
              ...
              {!fy.isCurrent && (
                <button
                  ...
                  className="w-full py-2.5 border-2 border-teal-500 text-teal-600 dark:text-teal-200 rounded-xl hover:bg-teal-50 dark:hover:bg-teal-950/25 transition-colors font-medium flex items-center justify-center gap-2"
                >
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Create Fiscal Year">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1.5">End Date</label>
            <input
              type="date"
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              className="w-5 h-5 text-teal-500 border-slate-300 dark:border-slate-700 rounded focus:ring-teal-500"
            />
            <label htmlFor="isCurrent" className="text-sm font-medium text-slate-600 dark:text-slate-300">
